...
public class NoticeDAO {
	...
	public int updatetNotice(Connection conn, Notice notice) {
		PreparedStatement pstmt = null;
		int result = 0;
	...
	public List<Notice> selectNoticeList(Connection conn, int currentPage) {
		PreparedStatement pstmt = null;
		ResultSet rset = null;
		String query = "SELECT * FROM(SELECT ROW_NUMBER() OVER(ORDER BY NOTICE_NO DESC) ROW_NUM, NOTICE_TBL.* FROM NOTICE_TBL)"
	...
	public String generatePageNavi(int currentPage) {
		...
		int naviCountPerPage = 10; // 네비게이션 몇개씩 보여줄까~?
		...
		// endNavi값이 총 범위의 갯수보다 커지는것을 막아주는 코드
